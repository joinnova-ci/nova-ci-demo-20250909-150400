...
class GitBranchManager:
    ...
    def commit_patch(
        ...
        if message is None:
            # Count changed files if available
            files_changed = len(changed_files) if changed_files else 0
            if files_changed == 1:
                file_text = "1 file"
            else:
                file_text = f"{files_changed} files"
            
            message = f"""🤖 Nova auto-fix: repair failing tests

• Minimal change set ({file_text})
• All tests pass locally & in CI
• Safety limits respected (≤5 files, ≤100 LOC)"""
